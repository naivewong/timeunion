...
class OverlappingPartitionTest : public testing::Test {
 public:
  OverlappingPartitionTest() {}
  ...
  void Add(const char* smallest, const char* largest, int64_t time_boundary,
           int64_t time_interval, SequenceNumber smallest_seq = 100,
           SequenceNumber largest_seq = 100) {
    FileMetaData* f = new FileMetaData;
    f->number = files_.size() + 1;
    f->smallest = InternalKey(smallest, smallest_seq, kTypeValue);
    f->largest = InternalKey(largest, largest_seq, kTypeValue);
    f->time_boundary = time_boundary;
    f->time_interval = time_interval;
    if (files_.empty() || time_boundary != files_.back()->time_boundary) {
      time_partitions_.push_back(time_boundary);
      time_partition_indexes_.push_back(files_.size());
    }
    files_.push_back(f);
  }

  void OverlappingPartitions(int64_t mint, int64_t maxt,
                             std::vector<std::pair<int64_t, int64_t>>* v,
                             std::vector<int>* indexes) {
    // Find upper bound (right boundaries) for mint.
    // Note(Alec), the first element >= mint.
    int mid;
    int low = 0;
    int high = time_partitions_.size();
    while (low < high) {
      mid = low + (high - low) / 2;
      if (mint >= time_partitions_[mid] +
                      files_[time_partition_indexes_[mid]]->time_interval)
        low = mid + 1;
      else
        high = mid;
    }
    ...
    low = 0;
    high = time_partitions_.size();
    while (low < high) {
      mid = low + (high - low) / 2;
      if (maxt >= time_partitions_[mid])
        low = mid + 1;
      else
        high = mid;
    }
    ...
    for (int i = left; i <= right; i++) {
      indexes->push_back(time_partition_indexes_[i]);
      v->emplace_back(time_partitions_[i],
                      files_[time_partition_indexes_[i]]->time_interval);
    }
  }

 private:
  std::vector<FileMetaData*> files_;
  std::vector<int64_t> time_partitions_;
  std::vector<int> time_partition_indexes_;
};

TEST_F(OverlappingPartitionTest, Test1) {
  Add("0", "100", 0, 100);
  Add("0", "100", 0, 100);
  Add("0", "100", 0, 100);
  Add("0", "100", 100, 100);
  ...
  std::vector<std::pair<int64_t, int64_t>> v;
  std::vector<int> indexes;
  OverlappingPartitions(100, 200, &v, &indexes);
  std::vector<std::pair<int64_t, int64_t>> r = {
      {(int64_t)(100), (int64_t)(100)}, {(int64_t)(200), (int64_t)(100)}};
  ASSERT_EQ(r, v);
  ASSERT_EQ(std::vector<int>({3, 6}), indexes);

  v.clear();
  indexes.clear();
  OverlappingPartitions(50, 250, &v, &indexes);
  r = {{(int64_t)(0), (int64_t)(100)},
       {(int64_t)(100), (int64_t)(100)},
       {(int64_t)(200), (int64_t)(100)}};
  ASSERT_EQ(r, v);
  ASSERT_EQ(std::vector<int>({0, 3, 6}), indexes);

  v.clear();
  indexes.clear();
  OverlappingPartitions(-1, 350, &v, &indexes);
  r = {{(int64_t)(0), (int64_t)(100)},
       {(int64_t)(100), (int64_t)(100)},
       {(int64_t)(200), (int64_t)(100)}};
  ASSERT_EQ(r, v);
  ...
  v.clear();
  indexes.clear();
  OverlappingPartitions(101, 199, &v, &indexes);
  r = {{(int64_t)(100), (int64_t)(100)}};
  ASSERT_EQ(r, v);
  ASSERT_EQ(std::vector<int>({3}), indexes);
}
...
int main(int argc, char** argv) {
  ...
  ::testing::GTEST_FLAG(filter) = "OverlappingPartitionTest*";
