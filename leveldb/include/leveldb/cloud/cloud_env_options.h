...
class CloudEnvOptions {
 ...
 public:
  ...
  CloudEnvOptions(
      ...
      bool _use_aws_transfer_manager = false, int _keep_sst_levels = -1)
      : cloud_type(_cloud_type),
        ...
        keep_sst_levels(_keep_sst_levels) {}
...
class CloudEnv : public Env {
 ...
 public:
  ...
  virtual std::string Name() override { return "cloud"; }
  // Returns the underlying env
  virtual Env* GetBaseEnv() override { return base_env_; }
  ...
  virtual CloudEnvOptions* GetCloudEnvOptionsPtr() override {
    return &cloud_env_options;
  }
  ...
  static Status TEST_NewAwsEnv(
      Env* base_env, const std::string& src_bucket_name,
      const std::string& src_object_prefix,
  ...
  static Status TEST_NewAwsEnv(Env* base_env,
                               const CloudEnvOptions& env_options,
                               const std::shared_ptr<Logger>& logger,
                               AwsEnv** cenv);
};
