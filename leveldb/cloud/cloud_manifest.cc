...
Status CloudManifest::LoadFromLog(std::unique_ptr<SequentialFile> log,
                                  std::unique_ptr<CloudManifest>* manifest) {
  Status status;
  CorruptionReporter reporter;
  reporter.status = &status;
  log::Reader reader(log.get(), &reporter, true /* checksum */,
                     0 /* log_num */);
  Slice record;
  std::string scratch;
...
Status CloudManifest::WriteToLog(std::unique_ptr<WritableFile> log) {
  Status status;
  log::Writer writer(log.get());
  std::string record;
