...
inline bool IsSstValueLog(const std::string& pathname) {
  if (pathname.size() < vlog.size()) {
    return false;
  }
  const char* ptr = pathname.c_str() + pathname.size() - vlog.size();
  if ((memcmp(ptr, vlog.c_str(), vlog.size()) == 0)) {
    return true;
  }
  return false;
}
...
inline RocksDBFileType GetFileType(const std::string& fname_with_epoch) {
  ...
  if (IsSstValueLog(fname)) {
    return RocksDBFileType::kSstValueLog;
  }
  if (IsLogFile(fname)) {
