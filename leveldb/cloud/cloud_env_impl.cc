...
Status CloudEnvImpl::LoadLocalCloudManifest(const std::string& dbname) {
  ...
  return CloudManifest::LoadFromLog(std::unique_ptr<SequentialFile>(file),
                                    &cloud_manifest_);
}
...
std::string CloudEnvImpl::RemapFilename(const std::string& logical_path) const {
  ...
  // } else {
  //   bool ok = ParseFileName(file_name, &fileNumber, &type);
  //   if (!ok) {
  //     return logical_path;
  //   }
  // }
  ...
  //     // loaded
  //     assert(cloud_manifest_);
  //     // Even though logical file might say MANIFEST-000001, we cut the
  //     number
...
Status CloudEnvImpl::writeCloudManifest(CloudManifest* manifest,
  ...
  if (s.ok()) {
    s = manifest->WriteToLog(std::unique_ptr<WritableFile>(file));
  }
  if (s.ok()) {
...
Status CloudEnvImpl::NeedsReinitialization(const std::string& local_dir,
  ...
  *do_reinit = true;
  ...
  auto s = env->FileExists(local_dir);
  if (!s) {
    Log(InfoLogLevel::ERROR_LEVEL, info_log_,
        "[cloud_env_impl] NeedsReinitialization: "
        "failed to access local dir %s",
        local_dir.c_str());
  ...
  s = env->FileExists(CurrentFileName(local_dir));
  if (!s) {
    Log(InfoLogLevel::ERROR_LEVEL, info_log_,
        "[cloud_env_impl] NeedsReinitialization: "
        "failed to find CURRENT file %s",
        CurrentFileName(local_dir).c_str());
    return Status::NotFound();
  }
  ...
  auto st = ReadFileToString(env, IdentityFileName(local_dir), &local_dbid);
  if (!st.ok()) {
    Log(InfoLogLevel::ERROR_LEVEL, info_log_,
        "[cloud_env_impl] NeedsReinitialization: "
...
Status CloudEnvImpl::MaybeMigrateManifestFile(const std::string& local_dbname) {
  ...
  Status s = ReadFileToString(local_env, CurrentFileName(local_dbname),
                              &manifest_filename);
  if (!s.ok()) {
    return s;
  }
  ...
  if (!local_env->FileExists(manifest_filename)) {
...
//
Status CloudEnvImpl::SanitizeDirectory(const Options& options,
                                       const std::string& local_name,
                                       bool read_only) {
  EnvOptions soptions;
...
Status CloudEnvImpl::FetchCloudManifest(const std::string& local_dbname,
  ...
  if (!SrcMatchesDest() && !force && GetBaseEnv()->FileExists(cloudmanifest)) {
...
Status CloudEnvImpl::RollNewEpoch(const std::string& local_dbname) {
  ...
  if (maxFileNumber > 0) {
    ...
    st = CopyFile(GetBaseEnv(), ManifestFileWithEpoch(local_dbname, oldEpoch),
                  ManifestFileWithEpoch(local_dbname, newEpoch), 0);
    if (!st.ok()) {
      return st;
    }
  }
