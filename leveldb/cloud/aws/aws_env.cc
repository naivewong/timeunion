...
AwsEnv::AwsEnv(Env* underlying_env, const CloudEnvOptions& _cloud_env_options,
  ...
  if (!SrcMatchesDest() && HasSrcBucket() && HasDestBucket()) {
    if (cloud_env_options.src_bucket.GetRegion() ==
        cloud_env_options.dest_bucket.GetRegion()) {
      // alls good
    } else {
      create_bucket_status_ =
  ...
  create_bucket_status_ = AwsCloudOptions::GetClientConfiguration(
      this, cloud_env_options.src_bucket.GetRegion(), &config);
  ...
  if (!create_bucket_status_.ok()) {
    Log(InfoLogLevel::ERROR_LEVEL, info_log,
        ...
        GetDestBucketName().c_str(), create_bucket_status_.ToString().c_str());
  }
  ...
  if (create_bucket_status_.ok() && !cloud_env_options.keep_local_log_files) {
    if (cloud_env_options.log_type == kLogKinesis) {
      // create_bucket_status_ = CreateKinesisController(this,
...
AwsEnv::~AwsEnv() {
  ...
  std::cout << "put_request_count: " << put_request_count << std::endl;
...
Status AwsEnv::NewSequentialFileCloud(const std::string& bucket,
  ...
  Status st = NewS3ReadableFile(bucket, fname, &file, options);
  if (!st.ok()) {
    return st;
  }
...
Status AwsEnv::PrefetchFile(const std::string& logical_fname, bool remap) {
  assert(status().ok());
  auto fname = logical_fname;
  if (remap) fname = RemapFilename(logical_fname);
  if (base_env_->FileExists(fname)) return Status::OK();
  Status st;
  if (HasDestBucket()) {
  ...
  Log(InfoLogLevel::DEBUG_LEVEL, info_log_, "[aws] PrefetchFile file %s %s",
...
Status AwsEnv::NewSequentialFile(const std::string& logical_fname,
  ...
  bool sstfile = (file_type == RocksDBFileType::kSstFile),
       sstvlog = (file_type == RocksDBFileType::kSstValueLog),
  ...
  if (sstfile || sstvlog || manifest || identity) {
    ...
    if (!st.ok()) {
      if ((cloud_env_options.keep_local_sst_files &&
           cloud_env_options.keep_sst_levels == -1) ||
          (!sstfile && !sstvlog)) {
        ...
        if (st.ok()) {
          *result = dynamic_cast<SequentialFile*>(file.release());
        }
      }
    }
...
Status AwsEnv::NewRandomAccessFile(const std::string& logical_fname,
  ...
  if (sstfile || sstvlog || manifest || identity) {
    ...
    if (!st.ok() && base_env_->FileExists(fname)) {
      // if status is not OK, but file does exist locally, something is wrong
      return st;
    }
    // printf("AwsEnv::NewRandomAccessFile:%s sstfile:%d keep_local_sst_files:%d
    // keep_sst_levels:%d st:%d\n", fname.c_str(), sstfile,
    // cloud_env_options.keep_local_sst_files,
    // cloud_env_options.keep_sst_levels, st.ok());
    ...
    if ((cloud_env_options.keep_local_sst_files &&
      ...
      if (st.ok()) {
        *result = dynamic_cast<RandomAccessFile*>(file.release());
      }
    }
    Log(InfoLogLevel::DEBUG_LEVEL, info_log_,
...
Status AwsEnv::NewWritableFile(const std::string& logical_fname,
  ...
  if (HasDestBucket() && (sstfile || sstvlog || identity || manifest)) {
    ...
    *result = dynamic_cast<WritableFile*>(f.release());
  } else if (logfile && !cloud_env_options.keep_local_log_files) {
...
Status AwsEnv::NewStringWritableFile(const std::string& logical_fname,
                                     WritableFile** result,
                                     const EnvOptions& options) {
  assert(status().ok());
  ...
  if (HasDestBucket() && (sstfile || sstvlog || identity || manifest)) {
    std::unique_ptr<S3StringWritableFile> f(
        new S3StringWritableFile(this, GetDestBucketName(), destname(fname),
                                 options, cloud_env_options));
    s = f->status();
    if (!s.ok()) {
      Log(InfoLogLevel::ERROR_LEVEL, info_log_,
          "[s3] NewStringWritableFile src %s %s", fname.c_str(),
          s.ToString().c_str());
      return s;
    }
    *result = dynamic_cast<WritableFile*>(f.release());
    ...
    std::unique_ptr<CloudLogWritableFile> f(
        cloud_log_controller_->CreateWritableFile(fname, options));
    if (!f || !f->status().ok()) {
      s = Status::IOError("[aws] NewStringWritableFile", fname.c_str());
      Log(InfoLogLevel::ERROR_LEVEL, info_log_,
          "[kinesis] NewStringWritableFile src %s %s", fname.c_str(),
          s.ToString().c_str());
      return s;
    }
    *result = dynamic_cast<WritableFile*>(f.release());
  ...
  Log(InfoLogLevel::DEBUG_LEVEL, info_log_,
      "[aws] NewStringWritableFile src %s %s", fname.c_str(),
      s.ToString().c_str());
  return s;
}
...
//
bool AwsEnv::FileExists(const std::string& logical_fname) {
  assert(status().ok());
  Status st;
  ...
  if (sstfile || sstvlog || manifest || identity) {
    // We read first from local storage and then from cloud storage.
    bool found = base_env_->FileExists(fname);
    if (!found && HasDestBucket()) {
      st = ExistsObject(GetDestBucketName(), destname(fname));
    }
    if (!found && HasSrcBucket()) {
    ...
    if (st.ok()) {
      ...
      auto lambda = [this, pathname]() -> Status {
        return (base_env_->FileExists(pathname) ? Status::OK()
                                                : Status::NotFound());
      };
      st = cloud_log_controller_->Retry(lambda);
    }
  } else {
    st = base_env_->FileExists(fname) ? Status::OK() : Status::NotFound();
  }
...
Status AwsEnv::HeadObject(const std::string& bucket, const std::string& path,
...
Status AwsEnv::NewS3ReadableFile(const std::string& bucket,
  ...
  result->reset(new S3ReadableFile(this, bucket, fname, size, options));
  return Status::OK();
}
...
Status AwsEnv::EmptyBucket(const std::string& bucket,
  ...
  Log(InfoLogLevel::DEBUG_LEVEL, info_log_,
      "[s3] EmptyBucket going to delete %"
      "zu"
      " objects in bucket %s",
...
Status AwsEnv::GetChildren(const std::string& path,
  ...
  result->erase(std::remove_if(result->begin(), result->end(),
                               [&](const std::string& f) {
                                 ...
                                 if (!IsSstFile(noepoch) &&
                                     !IsSstValueLog(noepoch) &&
  ...
  for (size_t i = 0; i < result->size(); ++i) {
    ...
    if (IsSstFile(noepoch) || IsSstValueLog(noepoch) ||
        IsManifestFile(noepoch)) {
  ...
  *time = 0;
  ...
  if (sstfile || sstvlog) {
    if (base_env_->FileExists(fname)) {
      ...
      st = Status::NotFound();
      if (HasDestBucket()) {
        st = HeadObject(GetDestBucketName(), destname(fname), nullptr, nullptr,
                        time);
      }
      if (st.IsNotFound() && HasSrcBucket()) {
...
Status AwsEnv::GetObject(const std::string& bucket_name,
  ...
  if (!result.success) {
    ...
    std::string errmsg(error.GetMessage().c_str(), error.GetMessage().size());
    Log(InfoLogLevel::ERROR_LEVEL, info_log_, "[s3] GetObject %s/%s error %s.",
...
AwsEnv::PutObjectResult AwsEnv::DoPutObjectWithString(const std::string& input,
                                                      const Aws::String& bucket,
                                                      const Aws::String& key) {
  auto inputData = Aws::MakeShared<Aws::StringStream>(
      "PutObjectInputStream", std::stringstream::in | std::stringstream::out |
                                  std::stringstream::binary);
  inputData->write(input.c_str(), input.size());
  ...
  auto putOutcome = s3client_->PutObject(putRequest, input.size());
  PutObjectResult result;
  result.success = putOutcome.IsSuccess();
  if (!result.success) {
    result.error = putOutcome.GetError();
...
AwsEnv::PutObjectResult AwsEnv::DoPutObjectWithString(
    const std::shared_ptr<Aws::StringStream>& input, uint64_t size,
    const Aws::String& bucket, const Aws::String& key) {
  Aws::S3::Model::PutObjectRequest putRequest;
  putRequest.SetBucket(bucket);
  putRequest.SetKey(key);
  putRequest.SetBody(input);
  SetEncryptionParameters(cloud_env_options, putRequest);

  auto putOutcome = s3client_->PutObject(putRequest, size);
  PutObjectResult result;
  result.success = putOutcome.IsSuccess();
  if (!result.success) {
    result.error = putOutcome.GetError();
...
Status AwsEnv::PutObjectWithString(const std::string& input,
                                   const std::string& bucket_name,
                                   const std::string& object_path) {
  auto s3_bucket = ToAwsString(bucket_name);
  PutObjectResult result =
      DoPutObjectWithString(input, s3_bucket, ToAwsString(object_path));
  Status st;
  if (!result.success) {
    auto error = result.error;
    std::string errmsg(error.GetMessage().c_str(), error.GetMessage().size());
    st = Status::IOError(errmsg);
    Log(InfoLogLevel::ERROR_LEVEL, info_log_,
        "[s3] PutObjectWithString %s/%s, size %" PRIu64 ", ERROR %s",
        s3_bucket.c_str(), object_path.c_str(), input.size(), errmsg.c_str());
  } else {
    Log(InfoLogLevel::INFO_LEVEL, info_log_,
        "[s3] PutObjectWithString %s/%s, size %" PRIu64 ", OK",
        s3_bucket.c_str(), object_path.c_str(), input.size());
  }

  return st;
}

Status AwsEnv::PutObjectWithString(
    const std::shared_ptr<Aws::StringStream>& input, uint64_t size,
    const std::string& bucket_name, const std::string& object_path) {
  auto s3_bucket = ToAwsString(bucket_name);
  PutObjectResult result =
      DoPutObjectWithString(input, size, s3_bucket, ToAwsString(object_path));
  Status st;
  if (!result.success) {
    auto error = result.error;
    std::string errmsg(error.GetMessage().c_str(), error.GetMessage().size());
    ...
    Log(InfoLogLevel::ERROR_LEVEL, info_log_,
        "[s3] PutObjectWithString %s/%s, size %" PRIu64 ", ERROR %s",
        s3_bucket.c_str(), object_path.c_str(), size, errmsg.c_str());
  } else {
    Log(InfoLogLevel::INFO_LEVEL, info_log_,
        "[s3] PutObjectWithString %s/%s, size %" PRIu64 ", OK",
        s3_bucket.c_str(), object_path.c_str(), size);
  }
...
Status AwsEnv::NewLogger(const std::string& fname, Logger** result) {
...
Status AwsEnv::TEST_NewAwsEnv(Env* base_env,
