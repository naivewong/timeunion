...
class TestThreadPool {
 public:
  explicit TestThreadPool(std::size_t threads = (std::max)(
 ...
 private:
  ...
  struct handle_in_flight_decrement {
    ...
    handle_in_flight_decrement(TestThreadPool& tp_) : tp(tp_) {}
...
inline TestThreadPool::TestThreadPool(std::size_t threads)
...
template <class F, class... Args>
auto TestThreadPool::enqueue(F&& f, Args&&... args)
    -> std::future<typename std::result_of<F(Args...)>::type> {
  ...
  if (stop) throw std::runtime_error("enqueue on stopped TestThreadPool");
...
inline void TestThreadPool::set_pool_size(std::size_t limit) {
  if (limit < 1) limit = 1;
...
inline void TestThreadPool::start_worker(
